...
function sortByOrder(list) {
  return list.sort((a, b) => toNumber(a?.sortOrder) - toNumber(b?.sortOrder));
}
  ...
  return sortByOrder(
    rawSubjects
      ...
      .map((subject) => ({
        ...
        sortOrder: toNumber(subject.sortOrder),
      }))
  );
}
  ...
  return sortByOrder(
    rawSections
      ...
      .map((section) => ({
        ...
        sortOrder: toNumber(section.sortOrder),
      }))
  ...
  return sortByOrder(
    rawTopics
      ...
      .map((topic) => ({
        ...
        sortOrder: toNumber(topic.sortOrder),
      }))
    ...
    rawGuides
      ...
      .map((guide) => {
        ...
        return {
          ...
          featuredOrder: toNumber(guide.featuredOrder, toNumber(guide.sortOrder)),
          ...
          sortOrder: toNumber(guide.sortOrder),
        };
      })
  );
}
  ...
  return sortByOrder(
    rawLinks
      ...
      .map((link) => ({
        ...
        sortOrder: toNumber(link.sortOrder),
      }))
      .filter((link) => {
...
export function normalizeTipsPayload(rawTips, sites = []) {
  ...
  const curriculumSections = normalizeCurriculumSections(
    rawTips?.curriculumSections,
    subjectPairs,
    topicPairs
  );
...
function applyContentToWindow({ categories, sites, detailsBySiteKey, tips }) {
  if (categories && typeof categories === "object") {
    window.defaultCategories = categories;
  }

  if (Array.isArray(sites)) {
    window.initialSites = sites;
  }

  if (detailsBySiteKey && typeof detailsBySiteKey === "object") {
    window.siteDetailMap = { ...detailsBySiteKey };
  } else {
    window.siteDetailMap = {};
  }

  window.ddakpilmoContentTips = tips && typeof tips === "object"
    ? {
        subjects: Array.isArray(tips.subjects) ? tips.subjects : [],
        sections: Array.isArray(tips.sections) ? tips.sections : [],
        items: Array.isArray(tips.items) ? tips.items : [],
        curriculumGroups: Array.isArray(tips.curriculumGroups) ? tips.curriculumGroups : [],
        curriculumTopics: Array.isArray(tips.curriculumTopics) ? tips.curriculumTopics : [],
        curriculumSections: Array.isArray(tips.curriculumSections) ? tips.curriculumSections : [],
        curriculumItems: Array.isArray(tips.curriculumItems) ? tips.curriculumItems : [],
        guides: Array.isArray(tips.guides) ? tips.guides : [],
        guideSections: Array.isArray(tips.guideSections) ? tips.guideSections : [],
        guideLinks: Array.isArray(tips.guideLinks) ? tips.guideLinks : [],
      }
    : { ...EMPTY_TIPS };
}

async function loadContentFromRemote({ cacheMinutes = 0, preferCache = false } = {}) {
  const now = Date.now();
  let cached = null;
  ...
  let res;
  try {
    res = await fetch(getContentApiUrl(), { cache: "no-store", signal: AbortSignal.timeout(6000) });
  } catch (error) {
    if (cached?.data) {
...
async function loadLocalSnapshot() {
  const [categoriesRes, sitesRes, contentRes] = await Promise.all([
  ...
  return {
    ...
    detailsBySiteKey: normalizeDetailsMap(contentExample),
    rawTips: contentExample?.tips || null,
  };
}

async function loadJSONData({ cacheMinutes = 0 } = {}) {
  let remotePayload = null;

  try {
    remotePayload = await loadContentFromRemote({ cacheMinutes });
  } catch (error) {
  ...
  try {
    if (remotePayload?.hasContentShape && remotePayload.categories && remotePayload.sites) {
      ...
      console.log("content API data loaded", {
        categories: Object.keys(window.defaultCategories || {}).length,
        sites: Array.isArray(window.initialSites) ? window.initialSites.length : 0,
        details: Object.keys(window.siteDetailMap || {}).length,
        curriculumGroups: window.ddakpilmoContentTips?.curriculumGroups?.length || 0,
        guides: window.ddakpilmoContentTips?.guides?.length || 0,
      });
      return;
    }
    ...
    applyContentToWindow({
      ...
      detailsBySiteKey: { ...localSnapshot.detailsBySiteKey, ...remotePayload?.detailsBySiteKey },
      tips: hasTipsContent(remoteTips) ? remoteTips : localTips,
    });
    window.__siteDataContentLoaded = false;

    console.log("local snapshot data loaded", {
      categories: Object.keys(window.defaultCategories || {}).length,
      sites: Array.isArray(window.initialSites) ? window.initialSites.length : 0,
      details: Object.keys(window.siteDetailMap || {}).length,
      curriculumGroups: window.ddakpilmoContentTips?.curriculumGroups?.length || 0,
...
async function loadDetailsFromSheet({ cacheMinutes = 0 } = {}) {
  if (window.__siteDataContentLoaded && Object.keys(window.siteDetailMap || {}).length > 0) {
    return window.siteDetailMap;
  }

  const remotePayload = await loadContentFromRemote({ cacheMinutes });
...
export function installSiteData() {
  if (
    typeof window.loadJSONData === "function" &&
    typeof window.loadDetailsFromSheet === "function"
  ) {
    return {
      CONTENT_API_URL: window.CONTENT_API_URL,
      DETAILS_API_URL: window.DETAILS_API_URL,
      loadContentFromRemote: window.loadContentFromRemote,
      clearContentCache: window.clearContentCache,
      refreshContentFromSheet: window.refreshContentFromSheet,
      loadLocalSnapshot: window.loadLocalSnapshot,
      loadJSONData: window.loadJSONData,
      loadDetailsFromSheet: window.loadDetailsFromSheet,
    };
  }
  ...
  return {
    ...
    clearContentCache,
    refreshContentFromSheet,
    loadLocalSnapshot,
    loadJSONData,
    loadDetailsFromSheet,
  };
}
