function createCategorySection(categoryKey) {
  const allCategories = typeof window.getAllCategories === "function" ? window.getAllCategories() : {};
...
function renderCategorySections() {
  ...
  if (!container.dataset.initialized) {
    container.innerHTML = "";
    const keys = Object.keys(typeof window.getAllCategories === "function" ? window.getAllCategories() : {});
    keys.forEach((key) => {
...
export function installSections() {
  if (typeof window.createCategorySection === "function" && typeof window.renderCategorySections === "function") {
    return {
      createCategorySection: window.createCategorySection,
      renderCategorySections: window.renderCategorySections,
      renderSitesLegacy: window.renderSitesLegacy,
    };
  }
  ...
  return { createCategorySection, renderCategorySections, renderSitesLegacy };
}
