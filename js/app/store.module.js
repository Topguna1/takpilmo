...
function createStore() {
  ...
  let state = {
    ...
    retention: {
      ...INITIAL_STATE.retention,
      ...(window.state?.retention || {}),
      profile: {
        ...INITIAL_STATE.retention.profile,
        ...(window.state?.retention?.profile || {}),
      },
    },
  };
    ...
    const nextState = {
      ...
      settings:
        nextPatch.settings !== undefined
          ...
          : state.settings,
      retention:
        nextPatch.retention !== undefined
          ? {
            ...state.retention,
            ...nextPatch.retention,
            profile:
              nextPatch.retention.profile !== undefined
                ? { ...state.retention.profile, ...nextPatch.retention.profile }
                : state.retention.profile,
          }
          : state.retention,
    };
...
export function installStore() {
  ...
  if (window.App.store?.getState && typeof window.App.store.setState === "function") {
  ...
  window.setFilters = store.setFilters;
  window.setPage = store.setPage;
  return store;
}
