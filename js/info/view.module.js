...
async function admin(root, sites, live) {
  ...
  if (!live()) return;
  root.innerHTML = `<h1>딱필 정보 관리</h1><div class="info-toolbar"><button data-info-new>새 글 작성</button><button data-info-seed>초기 글 8편 등록</button><button data-info-logout>로그아웃</button></div><p id="infoAdminNotice" role="status"></p><div class="info-admin-list">${items.map(item => `<button data-info-edit="${esc(item.id)}">${esc(item.title)} · ${item.status === 'published' ? '공개' : '초안'}</button>`).join('')}</div><div id="infoEditor"></div>`;
  ...
  function openEditor(item) {
    ...
    form.onsubmit = async event => {
      ...
      } catch (error) { notice.textContent = error.message || '저장하지 못했어요. 다시 시도하세요.'; }
      finally { pendingSave = false; form.querySelectorAll('input,textarea,select,button').forEach(control => control.disabled = false); }
    };
