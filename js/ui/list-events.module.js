...
function setupEventListeners() {
  ...
  if (mainPanel) {
    add(mainPanel, "click", (e) => {
      const retentionBtn = e.target.closest("[data-retention-action]");
      if (retentionBtn) {
        e.preventDefault();
        e.stopPropagation();
        const action = retentionBtn.getAttribute("data-retention-action");
        const key = retentionBtn.getAttribute("data-site-key");
        window.ddakpilmo?.retention?.handleAction?.(action, key);
        return;
      }
  ...
  add(document, "click", (e) => {
    const retentionBtn = e.target.closest("#detailView [data-retention-action]");
    if (retentionBtn) {
      e.preventDefault();
      e.stopPropagation();
      const action = retentionBtn.getAttribute("data-retention-action");
...
export function installListEvents() {
  if (window.setupEventListeners && typeof window.resetFilters === "function") {
    return {
      setupEventListeners: window.setupEventListeners,
      resetFilters: window.resetFilters,
      shareSite: window.shareSite,
      updateSelectedFiltersSummary: window.updateSelectedFiltersSummary,
    };
  }
  ...
  return { setupEventListeners, resetFilters, shareSite, updateSelectedFiltersSummary };
}
